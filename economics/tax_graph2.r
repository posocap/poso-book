...
require(tidyverse)
require(econocharts)
tax_graph2 <- function(demand_fun, supply_fun, supply_tax, names = c("", 
                                                        "", "DWL", "", ""), 
          ...
          bg.col = "white") 
{
  ...
  full_plot <- base_plot + geom_segment(aes(x = equilibrium, 
                                            xend = equilibrium, y = -Inf, yend = supply_fun(equilibrium)), 
                                        color = "grey50", size = 0.5, linetype = "dashed")   + 
    geom_segment(aes(x = -Inf, xend = equilibrium_tax, y = supply_tax(equilibrium_tax), 
                     yend = supply_tax(equilibrium_tax)), color = "grey50", 
                 size = 0.5, linetype = "dashed") + geom_segment(aes(x = -Inf, 
                                                                     xend = equilibrium_tax, y = supply_fun(equilibrium_tax), 
                                                                     yend = supply_fun(equilibrium_tax)), color = "grey50", 
                                                                 size = 0.5, linetype = "dashed") + stat_function(fun = supply_fun, 
                                                                                                                  size = 1.5, color = nord_red) + stat_function(fun = supply_tax, 
                                                                                                                                                                size = 1.5, color = nord_orange) + stat_function(fun = demand_fun, 
                                                                                                                                                                                                                 size = 1.5, color = nord_dk_blue) + annotate(geom = "label", 
                                                                                                                                                                                                                                                              x = 38, y = supply_fun(38), label = "MC", size = 4, fill = nord_red, 
                                                                                                                                                                                                                                                              color = "white") + annotate(geom = "label", x = 38, y = demand_fun(38), 
                                                                                                                                                                                                                                                                                                                   label = "D", size = 4, fill = nord_dk_blue, color = "white") + 
                                                                                                                        ...
                                                                                                                        ylim) + labs(x = xlab, y = ylab, title = title) + theme_econ(13, 
                                                                                                                                                                                     axis_line = TRUE) + theme(panel.grid = element_blank(), 
                                                                                                                                                                                                               plot.background = element_rect(fill = bg.col), plot.margin = margin(0.5, 
                                                                                                                                                                                                                                                                                   1, 0.5, 0.5, "cm"))
  # Adding MR line and label
  mr_start_x <- 0
  mr_end_x   <- 25
  mr_start_y <- 20
  mr_end_y   <- 2.5
  
  # Adding to the plot within the function
  full_plot <- full_plot +
    geom_segment(aes(x = mr_start_x, y = mr_start_y, xend = mr_end_x, yend = mr_end_y),
                 color = "darkgreen", size = 1.5, linetype = "dashed") +
    annotate("label", x = mr_end_x, y = mr_end_y, label = "MR", 
             color = "white", fill = "darkgreen", fontface = "bold", size = 4)
  
  if (shaded) {
    final_plot <- full_plot + geom_label(data = surplus_labels[3,], 
